#!/usr/bin/env python3
"""Convert AVEVA/RMSS attribute TXT to route-materialized PSI116 XML.

This upstream converter emits guard PIPE nodes around every fitting/support
center node so unchanged xml_to_cii.py can detect bends/SIF/reducers/restraints
from branch order.
"""
...
from psi116_upstream_common import convert_attribute_txt


def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description='Convert AVEVA/RMSS attribute TXT to route-materialized PSI116 XML.')
    ...
    p.add_argument('--purpose', default='RMSS attribute TXT conversion')
    p.add_argument('--title-line', default='RMSS Attribute TXT Output')
...
def main() -> None:
    ...
    convert_attribute_txt(args.input, args.output, args)
