...
function ensureFileInput(root, labelId, inputId, caption, nameId, emptyText) {
  ...
  if (name && !input.files?.[0] && !/Mock .* loaded\./i.test(name.textContent || '')) name.textContent = emptyText;
}
...
function renderLauncher(root) {
  setLabelCaption(root.querySelector('#model-converters-primary-label'), 'InputXML Property Transfer');
  ...
  if (primaryName) primaryName.textContent = 'Click Open Popup, then choose both Source and Target InputXML or use Load mock.';
