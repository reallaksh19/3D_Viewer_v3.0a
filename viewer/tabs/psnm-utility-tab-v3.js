...
function installStyle() {
  if (document.getElementById('psnm-style-v3')) return;
  const style = document.createElement('style');
  style.id = 'psnm-style-v3';
  style.textContent = `
...
export function renderPSNM_UtilityTab(container, ctx = {}) {
  ...
  function onClick(event) {
    const tab = event.target?.closest?.('[data-psnm-tabbtn]')?.dataset?.psnmTabbtn;
    ...
    else if (action === 'copyMatch') copyText(objectRowsCsv(result.rows), ctx);
    else if (action === 'copyCandidates') copyText(objectRowsCsv(result.candidateRows), ctx);
    else if (action === 'copyCoverage') copyText(coverageCsv(result), ctx);
    else if (action === 'copyUser') copyText(objectRowsCsv(result.userLog), ctx);
    else if (action === 'copyDebug') copyText(objectRowsCsv(result.debugLog), ctx);
  }
