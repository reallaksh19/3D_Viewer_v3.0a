...
function triple(v, fb) { const p = String(v || '').split(',').map((x) => Number(x.trim())); return p.length === 3 && p.every(Number.isFinite) ? { xMm: p[0], yMm: p[1], zMm: p[2] } : { xMm: fb[0], yMm: fb[1], zMm: fb[2] }; }
...
function summary(result) {
  ...
  const coord = rows.filter((r) => String(r.matchType || '').startsWith('COORD_')).length;
  const nodeCovered = nodeCov.filter((r) => r.status === 'COVERED').length;
  const psCovered = psCov.filter((r) => r.status === 'COVERED').length;
  const warnings = (result.userLog || []).filter((r) => r.level === 'WARNING').length;
  const errors = (result.userLog || []).filter((r) => r.level === 'ERROR').length;
  return `<div class="psnm-summary"><div><b>${rows.length}</b><span>PS Rows</span></div><div><b>${exact}</b><span>Exact Bore</span></div><div><b>${coord}</b><span>Coord Only</span></div><div><b>${rows.filter((r) => r.matchType === 'NO_MATCH').length}</b><span>No Match</span></div><div><b>${nodeCovered}/${nodeCov.length}</b><span>Mandatory Node</span></div><div><b>${psCovered}/${psCov.length}</b><span>Mandatory PS</span></div><div><b>${h(result.boreMode || 'prefer')}</b><span>Bore Mode</span></div><div><b>${warnings}</b><span>Warnings</span></div><div><b>${errors}</b><span>Errors</span></div></div>`;
}
...
function helpBlock() { return `<div class="psnm-help"><b>Mandatory inputs are explicit and independent.</b><br>1) PS Table optional <b>Mandatory</b> column marks PS rows mandatory.<br>2) <b>Mandatory PS Name list</b> is a separate explicit PS coverage requirement.<br>3) Node Table optional <b>Mandatory</b> column and <b>Mandatory Node No list</b> are separate node coverage requirements.<br>4) If a Mandatory PS listed here is missing from PS Table, it appears as <b>MISSING_FROM_PS_TABLE</b>.</div>`; }
function anchorCard(form) {
  ...
  try {
    ...
    const nodePs = PSNM_transformNodeToPsPosition(node, transform, Number(form.decimals) || 2);
    const dE = Math.abs(ps.e - nodePs.e), dU = Math.abs(ps.u - nodePs.u), dS = Math.abs(ps.s - nodePs.s);
    status = dE <= 0.01 && dU <= 0.01 && dS <= 0.01 ? '<span class="psnm-ok">PASS</span>' : '<span class="psnm-error">FAIL</span>';
    transformText = `E=X+${n(transform.datumE)} | U=Y+${n(transform.datumU)} | S=Z+${n(transform.datumS)}`;
    deltaText = `ΔE=${n(dE)} | ΔU=${n(dU)} | ΔS=${n(dS)}`;
  } catch (e) { deltaText = h(e.message || e); }
  try { psFound = PSNM_parsePsRows(form.ps).some((r) => r.psName === form.anchorPsName) ? '<span class="psnm-ok">FOUND</span>' : psFound; } catch {}
  try { nodeFound = PSNM_parseNodeRows(form.node).some((r) => r.node === form.anchorNode) ? '<span class="psnm-ok">FOUND</span>' : nodeFound; } catch {}
...
function candidateTable(rows) {
  ...
  rows.forEach((r) => { if (!groups.has(r.psName)) groups.set(r.psName, []); groups.get(r.psName).push(r); });
  return `<div class="psnm-modal-body"><div class="psnm-ranking-note"><div>1. Coord type</div><div>2. Mandatory pair</div><div>3. Bore policy</div><div>4. Lowest max Δ</div><div>5. Lowest 3D Δ</div><div>6. Non-terminal + row order</div></div>${Array.from(groups.entries()).map(([ps, list]) => `<details class="psnm-candidate-card" open><summary>${h(ps)} <span class="psnm-candidate-meta">${list.length} candidate(s)</span></summary><div class="psnm-tablewrap"><table class="psnm-table"><thead><tr><th>Rank</th><th>Decision</th><th>Node</th><th>Occ.</th><th>PS Mand.</th><th>Node Mand.</th><th>Type</th><th>Bore</th><th>ΔE</th><th>ΔU</th><th>ΔS</th><th>Why</th></tr></thead><tbody>${list.map((r, i) => `<tr><td>${i + 1}</td><td class="psnm-decision-${h(String(r.decision || '').toLowerCase())}">${h(r.decision)}</td><td>${h(r.matchingNode)}</td><td>${h(r.occurrenceId)}</td><td>${yesNo(r.isMandatoryPs)}</td><td>${yesNo(r.isMandatoryNode)}</td><td>${badge(r.matchType)}</td><td>${h(r.boreStatus)}</td><td>${n(r.dxMm)}</td><td>${n(r.dyMm)}</td><td>${n(r.dzMm)}</td><td>${h(r.reason)}</td></tr>`).join('')}</tbody></table></div></details>`).join('')}</div>`;
}
...
function tabBtn(id, label, active) { return `<button class="psnm-tab-btn ${active === id ? 'active' : ''}" data-psnm-tabbtn="${id}">${h(label)}</button>`; }
...
async function copyText(text, ctx) { try { await navigator.clipboard.writeText(String(text || '')); ctx.showToast?.('Copied CSV.', 'success'); } catch (e) { ctx.showToast?.(`Copy failed: ${e.message || e}`, 'error'); } }
function tableCsvFromRows(rows) { return (rows || []).map((r) => Object.values(r).map(csv).join(',')).join('\n'); }
function coverageCsv(result) { return `Mandatory PS Coverage\n${tableCsvFromRows(result.mandatoryPsCoverageRows)}\n\nMandatory Node Coverage\n${tableCsvFromRows(result.mandatoryCoverageRows)}`; }
...
export function renderPSNM_UtilityTab(container, ctx = {}) {
  ...
  function syncForm() { Object.keys(form).forEach((key) => { if (container.querySelector(`[data-psnm-input="${key}"]`)) form[key] = readFormValue(key); }); }
  async function run() {
    try {
      syncForm();
      ...
      refreshResults();
      showTab('match');
      const mapped = result.rows.filter((r) => String(r.finalStatus || '').startsWith('MATCHED')).length;
  ...
  function onClick(e) {
    const tab = e.target?.closest?.('[data-psnm-tabbtn]')?.dataset?.psnmTabbtn;
    if (tab) { showTab(tab); return; }
    const action = e.target?.closest?.('[data-psnm-action]')?.dataset?.psnmAction;
    if (!action) return;
    if (action === 'open' && !container.querySelector('[data-psnm="modal"]')) container.insertAdjacentHTML('beforeend', modal());
    else if (action === 'close') container.querySelector('[data-psnm="modal"]')?.remove();
    else if (action === 'run') void run();
    else if (action === 'copyMatch') copyText(tableCsvFromRows(result.rows), ctx);
    else if (action === 'copyCandidates') copyText(tableCsvFromRows(result.candidateRows), ctx);
    else if (action === 'copyCoverage') copyText(coverageCsv(result), ctx);
    else if (action === 'copyUser') copyText(tableCsvFromRows(result.userLog), ctx);
    else if (action === 'copyDebug') copyText(tableCsvFromRows(result.debugLog), ctx);
  }
  function onInput(e) {
    const input = e.target?.closest?.('[data-psnm-input]');
    if (input) {
      form[input.dataset.psnmInput] = input.type === 'checkbox' ? input.checked : input.value;
      status = { ...status, phase: 'Edited', detail: 'Inputs changed. Run match to refresh output.', percent: Math.min(status.percent, 5) };
      const p = container.querySelector('[data-psnm-panel="input"]');
      if (p) p.innerHTML = inputPanel();
      const old = container.querySelector('.psnm-statusbar');
      if (old) old.outerHTML = statusBar(status, form);
      return;
    }
    if (!e.target?.matches?.('[data-psnm="debugSearch"], [data-psnm-level]')) return;
  ...
  function onKeydown(e) { if (e.key === 'Escape' && container.querySelector('[data-psnm="modal"]')) container.querySelector('[data-psnm="modal"]')?.remove(); }
  ...
  return () => { container.removeEventListener('click', onClick); container.removeEventListener('input', onInput); document.removeEventListener('keydown', onKeydown); };
}
