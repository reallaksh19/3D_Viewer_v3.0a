...
const files = {
  ...
  stateService: 'viewer/tabs/model-converters/workflow/services/xml-cii-workflow-state-service.js',
  masterService: 'viewer/tabs/model-converters/workflow/services/xml-cii-master-service.js',
  previewService: 'viewer/tabs/model-converters/workflow/services/xml-cii-preview-service.js',
  runService: 'viewer/tabs/model-converters/workflow/services/xml-cii-run-service.js',
  stagedSourceService: 'viewer/tabs/model-converters/workflow/services/xml-cii-staged-source-service.js',
  jsonTracePanel: 'viewer/tabs/model-converters/json-trace/json-trace-panel.js',
...
const stateService = read(files.stateService);
const masterService = read(files.masterService);
const previewService = read(files.previewService);
const runService = read(files.runService);
const stagedSourceService = read(files.stagedSourceService);
const jsonTracePanel = read(files.jsonTracePanel);
...
assert(workflowPopup.includes("state?.phaseId === 'json-trace'"), 'popup must skip legacy bridge rendering for JSON Trace because it is modular');
...
assert(workflowPopup.includes('renderXmlCiiJsonTracePanel'), 'popup must render JSON Trace panel directly from the modular panel');
assert(workflowPopup.includes('createXmlCiiMasterService') && workflowPopup.includes('createXmlCiiPreviewService') && workflowPopup.includes('createXmlCiiRunService'), 'popup module must call modular XML CII workflow services');
assert(workflowPopup.includes('xmlCiiWorkflowPhaseTabs'), 'popup module must use state service phase availability');
...
assert(workflowPopup.includes('xml-cii-popup-native-panels.js?v=20260625-service-1'), 'popup module must cache-bust the service native fallback renderer');
assert(workflowPopup.includes('json-trace/json-trace-panel.js?v=20260626-json-trace-source-1'), 'popup module must cache-bust the JSON Trace panel');
...
assert(workflowModal.includes('aria-disabled="true"'), 'modal must support disabled workflow phase tabs');

assert(bridge.includes('./legacy-adapter.js'), 'legacy adapter dependency must remain isolated to bridge boundary');
assert(bridge.includes('legacy-adapter.js?v=20260625-service-1'), 'workflow bridge must cache-bust the service legacy adapter');
...
assert(modelTab.includes('xml-cii-workflow-popup.js?v=20260625-service-1'), 'ModelConvertersTab must cache-bust the service workflow popup fix');
assert(workflowPopup.includes('data-native-preview-parsed-source'), 'popup fallback binding must persist the JSON Trace staged-source toggle');
...
assert(nativePanels.includes('Use JSON Trace staged source'), 'native Preview panel must expose JSON Trace staged-source toggle');
assert(legacyAdapter.includes('xmlCiiWorkflowParsedStagedSource'), 'legacy adapter must delegate staged-source resolution to the service boundary');
assert(stagedSourceService.includes('xmlCiiWorkflowParsedStagedSource'), 'staged-source service must own approved staged-source resolution');
assert(legacyAdapter.includes('json_trace_staged_source.json'), 'final XML->CII run must inject JSON Trace staged source as the secondary input when enabled');
assert(stateService.includes('Custom Input is only for generating XML'), 'state service must explain why Custom Input is disabled when XML is loaded');
assert(masterService.includes('createXmlCiiMasterService'), 'master service must expose explicit master/config operations');
assert(previewService.includes('createXmlCiiPreviewService'), 'preview service must expose preview/diagnostics/weight/support operations');
assert(runService.includes('createXmlCiiRunService'), 'run service must own final-run handoff boundary');
assert(jsonTracePanel.includes('JsonNode Trace') && jsonTracePanel.includes('XML Node Wise Trace'), 'JSON Trace panel must expose both trace views');
